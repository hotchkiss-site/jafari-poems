...
into the repo's self-contained `poems/*.poem` files (a `===meta===` TOML section + text sections). Written for the next agent doing this job.
...
## Writing the `===meta===` section

Every field in `schema.toml` must be present (empty string / empty array if unused). Field rules:

- **`id`** — must equal the `.poem` filename stem. Pick a lowercase two-word, hyphenated
...
is the sign the new `.poem` files parse correctly (the `===meta===` id should match the filename stem).
...
- [ ] Write the `===meta===` section (first section of the `.poem`) with all `schema.toml` fields; record `source` and a `notes` provenance line.
